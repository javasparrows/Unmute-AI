...
- 「保存」ボタンクリック → DBに新バージョン作成
- バージョン番号がインクリメントされる
- 保存後にページリロード → 最新バージョンが自動復元

## TC-17: バージョン復元

- バージョン履歴パネル → 過去バージョン「復元」クリック
- 原文, 翻訳, 言語, journal, providerがその時点の状態に復元
- sentence rangesも復元され、ハイライト同期が正確

## TC-18: ユーザー認証

- 未認証で / アクセス → /login へリダイレクト
- Googleログイン成功 → / (ダッシュボード) へリダイレクト
- ログアウト → /login へリダイレクト

## TC-19: ドキュメント管理

- ダッシュボードに自分のドキュメント一覧が表示される
- 「新規作成」→ /documents/[id] へ遷移
- ドキュメント削除 → 一覧から消える
- 他ユーザーのドキュメントは表示されない

## TC-20: 別端末アクセス

- ブラウザAで保存したドキュメント → ブラウザBでログイン → 同じ内容が表示される
